...
router.get('/:promotionId', function (req, res, next) {
    /**
     * TODO: The cookie of the user should be a random number generated by the server. We should implement this.
     * However, the sign is not very important so I just leave it here, will modify this if I have time in the furture.
     */
    var wxCookie = req.cookies['wx'];
    var telephone = '';
    if (wxCookie != null) {
        var userCookies = wxCookie.split(',');
        telephone = userCookies.length == 2 ? userCookies[1] : userCookies[0];
    }
    ...
    return merchantServiceInterface.requestPromotionInformation(req.params.promotionId).then(promotion => {
        ...
        return res.render('mobile/promotion.ejs', {
            rawParameters: {
                promotionId: req.params.promotionId,
                baseUrl: config['extranet-server-url'][current_env]
            },
...
router.post('/sign/:promotionId', function (req, res, next) {
    ...
    if (telephone === '') {
        return next(new Error('您尚未登录到拍范，请先登录。'));
    }

    return userServiceInterface.requestUserInformationByTelephone(telephone).then(user => {
        if (!user) {
            throw new Error('您的登录已失效，请重新登录。');
        }
        return merchantServiceInterface.postPromotionSign(req.params.promotionId, user.id, req.body).then(result => {
